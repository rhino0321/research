...
#test_first_zero_after_nonzero_index = (test_subseq_after_first_nonzero < 10).idxmax()
...
print(np.mean(ec), np.mean(test_ec), np.std(ec), np.std(test_ec))
...
GRU_L1 = GRU(units = 1, activation = 'sigmoid', return_sequences = False, dropout = 0.2)(lstm_input_reshaped)
...
index = range(0, 2000)
test_target_data = test_target_data * np.std(test_ec_rec) + np.mean(test_ec_rec)
test_predictions = lstm_test_predictions * np.std(test_ec_rec) + np.mean(test_ec_rec)
select_ori = test_target_data[0:2000]
